...
with engine.connect() as conn:
    ...
    # execute的几种用法，具体参考pymysql的用法，不需要commit操作
    conn.execute(
        "insert into tablename(id,url,title) values (1,'url1','title1');")
    conn.execute(
        "insert into tablename(id,url,title) values (%s,%s,%s);", 2, 'url2', 'title2')
    conn.execute(
        "insert into tablename(id,url,title) values (%s,%s,%s)", (3, "url3", "title"))
